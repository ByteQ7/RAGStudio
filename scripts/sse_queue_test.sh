...
# ==================== 配置参数 ====================
BASE_URL="${BASE_URL:-http://localhost:9090/api/ragstudio}"
...
for i in $(seq 1 "${CONCURRENCY}"); do
  LOG_FILE="${LOG_DIR}/ragstudio_chat_${i}.log"
  if [[ -f "${LOG_FILE}" ]]; then
    SIZE=$(du -h "${LOG_FILE}" | cut -f1)
    LINES=$(wc -l < "${LOG_FILE}")
    echo -e "   ${GREEN}✓${RESET} ${GRAY}ragstudio_chat_${i}.log${RESET} ${DIM}(${SIZE}, ${LINES} lines)${RESET}"
  else
    echo -e "   ${RED}✗${RESET} ${GRAY}ragstudio_chat_${i}.log${RESET} ${RED}[Missing]${RESET}"
  fi
done
