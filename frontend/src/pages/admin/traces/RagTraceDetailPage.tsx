...
const STATUS_COLORS: Record<StatusType, { dot: string; bar: string }> = {
  success: { dot: "bg-blue-500", bar: "bg-blue-400" },
  failed: { dot: "bg-red-500", bar: "bg-red-400" },
  running: { dot: "bg-amber-500", bar: "bg-amber-400" },
  default: { dot: "bg-gray-300", bar: "bg-gray-300" }
};
      ...
      >
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <span
              className={cn(
                  "truncate",
                  isRoot ? "text-sm font-semibold text-indigo-900" : "text-sm text-gray-700"
              )}
        ...
        <div className="flex items-center">
          <div className="relative w-full h-6 bg-gray-50 rounded overflow-hidden">
            {[25, 50, 75].map(p => (
                <div
                    key={p}
  ...
  return (
      <Card>
        <CardHeader className="py-3 px-4">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              <CardTitle className="text-sm font-medium text-gray-700 truncate" title={displayName}>
                {displayName}
              </CardTitle>
        ...
        <CardContent className="px-4 pb-4 pt-0 space-y-3">
          ...
          {node.extraData && (
              <div>
                <p className="text-xs font-medium text-gray-500 mb-1">额外数据</p>
                <pre className="text-xs bg-gray-50 border border-gray-200 rounded p-2 overflow-x-auto whitespace-pre-wrap break-all text-gray-700">
...
function DetailField({
  ...
  return (
      <div className="flex items-center gap-2 min-w-0">
        ...
        {copyable && (
            <Copy className="h-3 w-3 text-gray-300 shrink-0" />
        )}
      </div>
  );
}
...
export function RagTraceDetailPage() {
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5 text-sm">
              <Link to="/admin/traces" className="text-gray-500 hover:text-gray-700">
                链路追踪
              </Link>
  ...
  return (
      <div className="space-y-4 pb-8">
        ...
        <span
            className="font-mono cursor-pointer hover:text-gray-700 flex items-center gap-1 transition-colors"
        ...
        {/* 指标条 */}
        <div className="flex items-center bg-gray-50 rounded-lg border border-gray-200 divide-x divide-gray-200">
          <MetricItem
              icon={Clock}
              label="总耗时"
        ...
        <Card>
          ...
          <CardContent className="p-0">
            {timeline.nodes.length === 0 ? (
                ...
                <div>
                  <div className="grid grid-cols-[minmax(180px,1fr)_120px_2fr_100px] gap-4 px-4 py-2 text-xs font-medium text-gray-500 bg-gray-50 border-y border-gray-100">
                    <span>节点</span>
                    <span>类型</span>
                    <span>时间线</span>
