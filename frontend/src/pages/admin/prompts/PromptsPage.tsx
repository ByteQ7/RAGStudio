...
import { Eye, History, RefreshCw, RotateCcw, Save, Search, Terminal } from "lucide-react";
...
export function PromptsPage() {
  ...
  useEffect(() => {
    ...
    getPrompt(selectedKey)
      ...
      .catch((error) => toast.error(getErrorMessage(error, "加载提示词详情失败")))
      .finally(() => {
  ...
  const openHistory = async () => {
    ...
    try {
      ...
      setHistory(await getPromptHistory(selectedKey));
    } catch (error) {
    ...
    <div className="admin-page">
      <div className="admin-page-header">
        <div>
          ...
          <p className="admin-page-subtitle">统一管理 Agent 提示词，编辑后立即热重载，无需重启服务</p>
        </div>
      ...
      <div className="flex gap-4 items-stretch">
        ...
        <Card className="w-[380px] shrink-0">
          <CardContent className="pt-4">
            {loading ? (
              ...
              <div className="text-center py-8 text-muted-foreground text-sm">暂无匹配的提示词</div>
            ) : (
                ...
                {prompts.map((item) => (
                  <button
                    ...
                    className={`w-full text-left rounded-lg border px-3 py-2.5 transition-colors ${
                      selectedKey === item.key ? "border-primary bg-[var(--color-fill-quaternary)]" : "hover:bg-[var(--color-fill-quaternary)]"
                    }`}
                    style={{ borderColor: selectedKey === item.key ? "hsl(var(--primary))" : "var(--color-border-secondary)" }}
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-sm font-medium truncate" style={{ color: "var(--color-text)" }}>
                        {item.name}
                      </span>
                      <Badge className={`shrink-0 text-[10px] ${CATEGORY_BADGE_COLORS[item.category] || ""}`}>
                        {CATEGORY_LABELS[item.category] || item.category}
                      </Badge>
                    </div>
                    <div className="mt-1 flex items-center justify-between gap-2">
                      <span className="font-mono text-[11px] truncate" style={{ color: "var(--color-text-tertiary)" }}>
                        {item.key}
                      </span>
                      <div className="flex items-center gap-1.5 shrink-0">
                        {item.customized && (
                          <span className="text-[10px]" style={{ color: "hsl(var(--primary))" }}>已自定义</span>
                        )}
                        <span className="text-[10px]" style={{ color: "var(--color-text-tertiary)" }}>v{item.version}</span>
                        <span
        ...
        <Card className="flex-1 min-w-0">
          <CardContent className="pt-4">
            {!selectedKey ? (
              <div className="text-center py-16 text-muted-foreground text-sm">请选择左侧提示词进行编辑</div>
              ...
              <div className="space-y-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-[11px]" style={{ color: "var(--color-text-tertiary)" }}>
                      当前版本 v{detail.version} · 来源：{detail.source === "db" ? "DB（自定义）" : "classpath（默认）"}
                ...
                {detail.variables && (
                  <div className="rounded-lg border px-3 py-2 text-[11px]" style={{ borderColor: "var(--color-border-secondary)", background: "var(--color-fill-quaternary)" }}>
                    <span style={{ color: "var(--color-text-tertiary)" }}>可用占位符：</span>
                    <span className="font-mono" style={{ color: "hsl(var(--primary))" }}>{detail.variables}</span>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant="outline" size="sm" onClick={() => { setSlotsText(""); setPreviewResult(""); setPreviewOpen(true); }}>
                  ...
                  <Button onClick={handleSave} disabled={saving || !hasUnsaved} className="admin-primary-gradient">
        ...
        <DialogContent className="sm:max-w-[720px]">
          ...
          {historyLoading ? (
            ...
            <div className="max-h-[420px] overflow-y-auto space-y-2">
              {history.map((item) => (
                <div key={item.version} className="rounded-lg border p-3" style={{ borderColor: "var(--color-border-secondary)" }}>
                  ...
                  <pre className="whitespace-pre-wrap text-[11px] leading-relaxed max-h-[140px] overflow-y-auto font-mono" style={{ color: "var(--color-text-secondary)" }}>
                    {item.content}
                  </pre>
                </div>
              ))}
      ...
      <AlertDialog open={!!rollbackTarget} onOpenChange={() => setRollbackTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              将提示词回滚到 v{rollbackTarget?.version} 的内容，当前内容将写入历史，并立即热重载生效。是否继续？
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>取消</AlertDialogCancel>
            <AlertDialogAction onClick={handleRollback}>回滚</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={resetOpen} onOpenChange={setResetOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              将 {detail?.name}（{detail?.key}）重置为出厂默认内容，当前自定义内容将写入历史，并立即热重载生效。是否继续？
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>取消</AlertDialogCancel>
        ...
        <DialogContent className="sm:max-w-[720px]">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <pre className="min-h-[300px] whitespace-pre-wrap rounded-lg border p-3 text-xs leading-relaxed overflow-y-auto font-mono"
                style={{ borderColor: "var(--color-border-secondary)", background: "var(--color-fill-quaternary)", color: "var(--color-text)" }}>
