...
const CardTitle = ({ children }: { children: ReactNode }) => (
    <h3 className="mb-4 text-sm font-semibold text-gray-700">{children}</h3>
);
    ...
    <header className="mb-3 flex items-center justify-between">
      ...
      <div className="flex items-center gap-3">
        ...
        <Button
            ...
            onClick={onRefresh}
            disabled={loading}
            className="h-9 w-9 rounded-lg border-gray-200 bg-white text-gray-500 hover:text-gray-700"
        >
        ...
        <div className="mt-3 flex items-center gap-1.5 text-sm">
          {showChange ? (
              ...
              {change!.value > 0 ? "+" : ""}
                ...
                <span className="text-gray-400">较上周期</span>
              </>
          ) : (
        ...
        >
          ...
          {tooltip?.show && (
              <>
                ...
                {/* 标签 */}
                <div
                    className="pointer-events-none absolute z-10 -translate-x-1/2 whitespace-nowrap rounded-lg bg-gray-800 px-2.5 py-1.5 text-xs text-white shadow-lg"
                    style={{
                      left: tooltip.x,
          ...
          {items.map((item) => {
            ...
            return (
                <div key={item.label} className="space-y-1.5">
                  <div className="flex h-24 items-end rounded-md border border-gray-200 bg-white p-1.5">
                    <div
                        className={cn(
          ...
          {metrics.map((metric) => {
            ...
            return (
                <div key={metric.label} className="flex items-center justify-between py-2">
                  ...
                  <span className="text-sm font-semibold tabular-nums text-gray-700">{valueText}</span>
                </div>
            );
          })}
        </div>
...
const AIPerformanceCard = ({
  ...
  const ringColor = successRate >= 95 ? "#0D9488" : successRate >= 85 ? "#D97706" : "#EF4444";
  ...
  return (
      <DashCard>
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-gray-700">AI 性能</h3>
          <span
        ...
        <div className="flex justify-center py-3">
          <div className="relative">
            ...
            <span className="text-2xl font-bold" style={{ color: ringColor }}>
              ...
              <span className="mt-0.5 text-xs text-gray-400">成功率</span>
            </div>
          </div>
        </div>
...
const InsightCard = ({ item }: { item: InsightCardData }) => {
  ...
  return (
      <div className="rounded-xl bg-gray-50 p-3.5">
        ...
        <p className="mt-0.5 text-xs text-gray-400">归因：{item.context}</p>
        {item.action && (
            <p className="mt-1 text-xs font-medium text-gray-600">建议：{item.action}</p>
        )}
      </div>
  );
};
