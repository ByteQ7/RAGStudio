...
| **检索质量优化** | 知识库语义选择防误杀 + 分数簇感知动态 TopK + 多模态 Rerank（图片 base64 直传），59 题评测集问答准确率 95% |
...
检索后处理链（去重 → Rerank → 动态 TopK）进一步精排：文本/图片统一由多模态 Rerank 模型（如 qwen3-vl-rerank）语义打分（图片以 base64 data URI 直传，不依赖公网地址），分数簇感知动态 TopK 按分数分布决定送入 LLM 的条数。59 题内部评测集问答准确率 **95%**。
