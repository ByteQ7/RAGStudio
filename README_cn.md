...
Agent 回答中引用知识库内容时，`[^chunk_{id}]` 标记自动转换为编号 `[N]`，在回答正文中显示为蓝色可点击的引用链接。点击后展开底部引用列表并滚动到对应条目。

- **内联编号引用**：`[^chunk_{id}]` → `[N]`，按首次出现顺序编号，点击展开对应 Chunk
- **底部引用列表**：展示 `[N]` 编号、Chunk ID、所属知识库及文档名称，支持展开查看原文
- **复制 Chunk ID**：每条引用右侧提供复制按钮，仅复制 Chunk ID
- **后端过滤**：`fireCitations()` 仅推送被 `[^chunk_id]` 标记引用的 Chunk，未使用的不推送
- **持久化**：引用的 Chunk 随消息一起存入 `t_message.citations`，刷新页面后仍可查看

### 文档分块策略

目前内置三种分块策略，在知识库配置中可选：

**重叠分块（`fixed_size`）**
- 按固定字符数切分，相邻块之间有重叠（overlap），避免上下文在边界处断裂
- 默认块大小 512 字符，overlap 128 字符
- 切块时自动对齐句子边界（中英文句号、感叹号、问号），不会从句子中间断开
- 自动处理 URL 跨行断开的问题

**递归分块（`recursive`）**
- 参考 LangChain RecursiveCharacterTextSplitter 算法
- 多级分隔符从粗到细递归切分：`\n\n` → `\n` → `。` → `！` → `？` → `. ` → `，`
- 块太大时自动使用下一级分隔符细切，直到块大小符合要求
- 支持可配置 overlap

**结构感知分块（`structure_aware`）**
- 专为 Markdown 文档设计，识别标题、代码块、段落等结构
- 按最小/目标/最大三级预算打包块（默认 600/1400/1800 字符）
- 块内尽量保持语义完整，标题和正文不会分到两块里
- 支持 block-level overlap、末尾小块的自动向前合并
